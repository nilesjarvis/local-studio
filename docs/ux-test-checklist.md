...
- 🔧✅ **Reload mid-stream on a NEW chat no longer loses the turn** (`a44f5556`): the URL stayed `?…&new=<nonce>` after a turn started, so reloading restarted a blank chat and the conversation vanished from view. Root cause: `handlePiSessionIdChange` never updated the URL. **Fixed**: on piSessionId assignment, `history.replaceState` `?new=`→`?session=<piSessionId>` (invisible to Next's `useSearchParams`, so the running turn's nav effect never re-fires; side-chat pane excluded). **Verified live**: new chat → send → URL flips to `?session=…` at ~2s; reload → conversation reopens with its content (user msg + 3 blocks, no blank-chat state) instead of vanishing. (Whether the *live* stream keeps flowing after reload vs. loading settled history is the deeper Phase 3b/standalone-SSE question — but the data-loss is gone.)
