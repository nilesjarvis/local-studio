...
- [x] Inspect recipes explore, local model sync, content model, runtime panel, and recipe modal lifecycle hooks.
- [x] Replace explore pool hydration, recommendation loading, model search debounce, local model loading, download-completion sync, pinned recipe loading, recipe loading, runtime refresh, and llama.cpp config help loading with `useSyncExternalStore`.
- [x] Verify touched files no longer reference effect hooks.
- [x] Validate useEffect-removal slice.
- [x] Remove unused legacy effect wrapper surfaced by the pre-push cleanup gate.
- [x] Commit this slice.
